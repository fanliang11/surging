...
namespace Surging.Core.DeviceGateway.Runtime.Device.MessageCodec
{
    ...
    public class TopicMessageCodec
    {
        ...
        public static TopicMessageCodec ReportProperty => new TopicMessageCodec("/*/properties/report", typeof(ReadPropertyMessage), route => route.GroupName("属性上报").UpStream(true).DownStream(false).HttpMethod("Post").Description("上报物模型属性数据").Example("{\"properties\":[\"属性ID\"]}"));
        ...
        public static TopicMessageCodec ReadReplyProperty => new TopicMessageCodec("/*/properties/read/reply", typeof(ReadPropertyMessage), route => route.GroupName("读取属性回复").UpStream(true).DownStream(false).HttpMethod("Post").Description("设备响应下发读取属性指令").Example("{\"messageId\":\"消息ID,回复时需要一致.\",\"properties\":{\"属性ID\":\"属性值\"}}"));
        ...
        public static TopicMessageCodec WriteProperty => new TopicMessageCodec("/*/properties/write", typeof(ReadPropertyMessage), route => route.GroupName("修改属性").UpStream(false).DownStream(true).Description("下发修改属性指令")
                           .Example("{\"properties\":{\"属性ID\":\"属性值\"}}"));

        public static TopicMessageCodec WriteReplyProperty => new TopicMessageCodec("/*/properties/write/reply", typeof(ReadPropertyMessage), route => route.GroupName("修改属性回复").UpStream(true).DownStream(false).Description("设备响应下发修改属性指令")
                   .Example("{\"properties\":{\"属性ID\":\"属性值\"}}"));
