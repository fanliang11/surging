    ...
    public static class ContainerBuilderExtensions
    {
        ...
        private static void RegisterLocalInstance(string typeName, ContainerBuilder services)
        {
            ...
            foreach (var t in types)
            {
                ...
                services.Register(p => Activator.CreateInstance(t)).Named(attribute.Name.ToString(), typeof(ICacheClient<>));
            ...
            try
            {
                ...
                foreach (var t in types)
                {
                    foreach (var setting in bingingSettings)
                    {
                        ...
                        {
                            services.Register(p => Activator.CreateInstance(t, new object[] { setting.Id })).Named(string.Format("{0}.{1}", setting.Id, mapsetting.Name), typeof(ICacheProvider));
                        }
                    }
                    var attribute = t.GetTypeInfo().GetCustomAttribute<IdentifyCacheAttribute>();
                    if (attribute != null)
                        services.Register(p => Activator.CreateInstance(t)).Named(attribute.Name.ToString(), typeof(ICacheProvider));
                }
            }
            catch { }
        }
