   ...
   public class ZooKeeperMqttServiceRouteManager : MqttServiceRouteManagerBase, IDisposable
    {
        ...
        public ZooKeeperMqttServiceRouteManager(ConfigInfo configInfo, ISerializer<byte[]> serializer,
            ...
            ILogger<ZooKeeperMqttServiceRouteManager> logger) : base(stringSerializer)
        {
        ...
        protected override async Task SetRoutesAsync(IEnumerable<MqttServiceDescriptor> routes)
        {
            ...
            await CreateSubdirectory(_configInfo.MqttRoutePath);
        ...
        private async Task EnterRoutes()
        {
            if (_routes != null)
                return;
            _connectionWait.WaitOne();
            var watcher = new ChildrenMonitorWatcher(_zooKeeper, _configInfo.MqttRoutePath,
             async (oldChildrens, newChildrens) => await ChildrenChange(oldChildrens, newChildrens));
            if (await _zooKeeper.existsAsync(_configInfo.MqttRoutePath, watcher) != null)
            {
                var result = await _zooKeeper.getChildrenAsync(_configInfo.MqttRoutePath, watcher);
